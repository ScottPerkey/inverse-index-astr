...
class inverse:
	...
	def create_indexed_csv(self):	
		...
		third_dec_4tpwr=[[round(x,3) for x in sublist] for sublist in self.period_sorted_percsv]	
		
		transpose_write=list(zip(*third_dec_4tpwr))
		rows=[file_clean_g15]+[list(row) for row in transpose_write]
		with open('rounded_power_ZTF_3d.csv','w',newline='')as csvfile:
	...
	def create_binned_csv(self):
		...
		"""
		all_data=pd.read_csv('rounded_power_ZTF_3d.csv')
		...
		full_bcsv = [] 
		for idx,(power_val,file_list) in enumerate(zip(singular_fpv,full_binned_files)):
			files_str='|'.join(file_list)
			full_bcsv.append([power_val,files_str])
	...
	def search_pile_match(self,csv_output=False):
		...
		for i,files_str in enumerate(self.binned_read['associated_filename_list']):
			...
			if file_name in actual_list:
				self.rndd_peaks.append( self.binned_read['rounded_power'][i] )
				self.l_fle_name.append(files_str)
		print(f'The closest ranked light curves associated with {file_name} are below:')
		print('\n')
